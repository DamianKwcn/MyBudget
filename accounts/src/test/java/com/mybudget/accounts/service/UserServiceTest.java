...
public class UserServiceTest {
    ...
    @Test
    public void shouldIncreaseBalanceForIncome() {
        // GIVEN
        ...
        // WHEN
        userService.updateBalance(testUserWithBalance.getKeycloakSub(), amount, true);

        // THEN
        assertEquals(BigDecimal.valueOf(60.00), testUserWithBalance.getBalance());
        verify(userRepository, times(1)).save(testUserWithBalance);
    }

    @Test
    public void shouldDecreaseBalanceForExpense() {
        // GIVEN
        ...
        // WHEN
        userService.updateBalance(testUserWithBalance.getKeycloakSub(), amount, false);

        // THEN
        assertEquals(BigDecimal.valueOf(5.00), testUserWithBalance.getBalance());
        verify(userRepository, times(1)).save(testUserWithBalance);
    }
    ...
    @Test
    public void shouldInitializeBalanceToZeroIfNull() {
        // GIVEN
        User userWithNullBalance = new User();
        userWithNullBalance.setKeycloakSub("testSubNullBalance");
        userWithNullBalance.setUsername("test-user-null-balance");
        userWithNullBalance.setEmail("nullbalance@example.com");
        userWithNullBalance.setBalance(null);

        when(userRepository.findByKeycloakSub(userWithNullBalance.getKeycloakSub()))
                .thenReturn(Optional.of(userWithNullBalance));

        // WHEN
        userService.updateBalance(userWithNullBalance.getKeycloakSub(), BigDecimal.valueOf(30.00), true);

        // THEN
        assertEquals(BigDecimal.valueOf(30.00), userWithNullBalance.getBalance());
